...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      // Vérifier si l'utilisateur a des crédits disponibles (plan + bonus)
      if (quota.totalRemaining <= 0) {
        console.log('❌ [GENERATE] Quota exceeded:', { used: quota.used, limit: quota.limit, bonus: quota.bonusCredits, totalRemaining: quota.totalRemaining });
        return NextResponse.json(
          { 
            success: false, 
            message: `Limite de ${quota.limit} emails atteinte pour ce mois${quota.bonusCredits > 0 ? ' et tous vos bonus ont été utilisés' : ''}. Passez au plan PRO pour continuer ou parrainez des amis pour gagner des bonus !`,
            quotaExceeded: true,
            usage: { used: quota.used, limit: quota.limit, remaining: quota.remaining, bonusCredits: quota.bonusCredits, totalRemaining: quota.totalRemaining }
          },
          { status: 429 }
        );
      }
