...
export async function POST(req: NextRequest) {
  try {
    ...
    // 🚀 Invalider le cache d'optimisation des tokens
    try {
      await invalidateOptimizedCache(session.user.id);
      console.log(`🗑️ Cache d'optimisation invalidé suite à la modification de la config IA`);
    } catch (cacheError) {
      console.warn('⚠️ Erreur invalidation cache (non bloquant):', cacheError);
    }
