...
export async function POST(req: NextRequest) {
  try {
    ...
    // Mode interne (appelé depuis le backend) : utilise l'userId fourni
    if (internal && providedUserId) {
      userId = providedUserId;
      console.log(`🔓 Mode interne: traitement pour userId ${userId}`);
    } else {
    ...
    // Récupérer l'email avec son compte
    ...
    if (accountError || !account) {
      return NextResponse.json({ error: 'Compte email introuvable' }, { status: 404 });
    }
