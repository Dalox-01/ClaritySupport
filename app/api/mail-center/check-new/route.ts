        ...
        for (const rawMessage of messages) {
          try {
            ...
            const { data: existing } = await supabase
              .from('emails_cache')
              .select('id')
              .eq('external_message_id', parsed.id || parsed.message_id)
            ...
            if (!existing) {
              ...
              const { data: savedEmail } = await supabase
                ...
                .insert({
                  ...
                  external_message_id: parsed.id || parsed.message_id,
                  thread_id: parsed.thread_id || parsed.threadId,
              ...
              if (savedEmail) {
                ...
                // 🤖 Déclencher le traitement automatique si IA activée
                try {
                  // Vérifier si l'IA est activée pour cet utilisateur
                  ...
                  if (aiConfig?.enabled) {
                    console.log(`🤖 IA activée - Traitement automatique de l'email ${savedEmail.id}`);
                    
                    // Vérifier les limites
                    const limitCheck = await canSendAutoReply(userId);
                    if (limitCheck.allowed) {
                      // Déclencher le traitement en arrière-plan (ne pas attendre)
                      fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/mail-center/process-auto-reply`, {
                        method: 'POST',
                      ...
                      }).catch(err => console.error('Erreur traitement auto:', err));
                    } else {
                      console.log(`⚠️ Limite auto-reply atteinte: ${limitCheck.reason}`);
                    }
                  }
                } catch (aiError) {
                  console.error('Erreur vérification IA:', aiError);
                }
              }
            } else {
