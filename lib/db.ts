...
export async function getUserQuota(userId: string): Promise<{
  ...
  const { data: user, error } = await supabase
    .from('users')
    .select('plan, usage_count, bonus_credits')
    .eq('id', userId)
    .single();
  ...
  console.log(`📊 [DB] User found - Plan: ${user.plan}, Usage: ${user.usage_count}, Bonus: ${user.bonus_credits || 0}`);
  ...
  const totalRemaining = remaining + bonusCredits;

  console.log(`📊 [DB] Quota calculated - Plan: ${planType}, Limit: ${limit}, Used: ${used}, Remaining: ${remaining}, Bonus: ${bonusCredits}, Total: ${totalRemaining}`);
...
export async function incrementUsage(userId: string, tokensUsed: number = 0): Promise<void> {
  ...
  const { data: user } = await supabase
    .from('users')
    .select('usage_count, tokens_used, bonus_credits')
    .eq('id', userId)
    .single();
  ...
  const currentBonusCredits = user.bonus_credits || 0;
  
  // Si l'utilisateur a des bonus credits, les utiliser en priorité
  if (currentBonusCredits > 0) {
    // Décrémenter les bonus credits
    await supabase
      .from('users')
      .update({
        bonus_credits: currentBonusCredits - 1,
        tokens_used: (user.tokens_used || 0) + tokensUsed,
      })
      .eq('id', userId);
    
    console.log(`📊 [DB] Used 1 bonus credit. Remaining bonus: ${currentBonusCredits - 1}`);
  } else {
    // Sinon, utiliser le quota normal du plan
    await supabase
      .from('users')
      .update({
    ...
    console.log(`📊 [DB] Used 1 plan credit. Usage count: ${(user.usage_count || 0) + 1}`);
  }
}
